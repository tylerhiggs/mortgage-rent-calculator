...
export function useRetirement() {
    ...
    const retirementIraValueHome = computed(() => {
        ...
        const additionalContribution = rent.value > mortgageStore.monthlyPayment ? rent.value - mortgageStore.monthlyPayment : 0
        const totalContribution = monthlyContribution.value + additionalContribution

        const fromAfterMortgagePayments = yearsToRetirement.value > mortgageStore.term ?
            futureValueWithContributions(interestRate.value, yearsToRetirement.value - mortgageStore.term, mortgageStore.monthlyPayment)
            : 0

        const fromInitial = basicIraHelper(interestRate.value, yearsToRetirement.value, currentSavings.value)
        const fromContributions = futureValueWithContributions(interestRate.value, yearsToRetirement.value, totalContribution)
    ...
    const retirementIraValueRent = computed(() => {
        const additionalContribution = rent.value < mortgageStore.monthlyPayment ? mortgageStore.monthlyPayment - rent.value : 0
        const totalContribution = monthlyContribution.value + additionalContribution

        const fromRegularSavings = basicIraHelper(interestRate.value, yearsToRetirement.value, currentSavings.value)
        const fromWouldHaveBeenDownPayment = basicIraHelper(interestRate.value, yearsToRetirement.value, mortgageStore.downPayment)
        const fromContributions = futureValueWithContributions(interestRate.value, yearsToRetirement.value, totalContribution)
